...
python run/train_expert.py \
  ...
  -r 400 \
  -n "knee_expert" \
  --resume '../result/knee_expert/phase_4/model_epoch_14.pt' \
  -t '../Data/train/' \
  -v '../Data/val/' \
  --data-anatomy-mode 2 \
  --no-validation \
  ...
  \
  --mask-mode augment \
  --mask-types-prob 0.4 0.1 0.05 0.4 0.05 \
  --mask-acc-probs "4:0.48 8:0.52" \
  --mask-aug-schedule exp \
  --mask-aug-start-prob 0.5 \
  --mask-aug-end-prob 0.56 \
  --mask-aug-delay 0 \
  \
  --mr-aug-on \
  --mr-aug-start-prob 0.6 \
  --mr-aug-end-prob 0.66 \
  --mr-aug-schedule exp \
  --mr-aug-delay 0 \
  \
