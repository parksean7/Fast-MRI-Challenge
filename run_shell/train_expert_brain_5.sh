...
python run/train_expert.py \
  ...
  -r 400 \
  -n "brain_expert" \
  --resume '../result/brain_expert/phase_4/model_epoch_14.pt' \
  -t '../Data/train/' \
  -v '../Data/val/' \
  --data-anatomy-mode 1 \
  --no-validation \
  ...
  \
  --mask-mode augment \
  --mask-types-prob 0.25 0.4 0.05 0.25 0.05 \
  --mask-acc-probs "4:0.36 8:0.64" \
  --mask-aug-schedule exp \
  --mask-aug-start-prob 0.5 \
  --mask-aug-end-prob 0.5 \
  --mask-aug-delay 0 \
  \
  --mr-aug-on \
  --mr-aug-start-prob 0.7 \
  --mr-aug-end-prob 0.5 \
  --mr-aug-schedule exp \
  --mr-aug-delay 0 \
  \
